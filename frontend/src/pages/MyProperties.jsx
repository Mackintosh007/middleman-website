...
import noImage from "../assets/no-image.png"; // ✅ LOCAL FALLBACK
...
function PropertyDetails() {
  ...
  useEffect(() => {
    ...
    const fetchProperty = async () => {
      try {
        ...
        // ✅ CRITICAL FIX: HANDLE DELETED PROPERTY
  ...
  // ✅ MAIN IMAGE LOGIC (NO PLACEHOLDER.COM)
  const mainImage =
    images.length > 0
      ? images[0].image_url
      : noImage;
            ...
            {images.length > 1 && (
              <div className="grid grid-cols-3 gap-3 mt-4">
                {images.slice(1).map((img) => (
                  <div
                    key={img.id}
                    className="h-28 bg-gray-100 rounded border flex items-center justify-center"
                  >
                    <img
                      src={img.image_url}
                      alt={property.title}
                      className="max-w-full max-h-full object-contain cursor-pointer hover:opacity-80"
                    />
                  </div>
                ))}
              </div>
